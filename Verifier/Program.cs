...
namespace Verifier
{
    public class Program
    {
        ...
        private static async Task Menu(DateTime startTime)
        {
            int runType = 0;
            do
            {
        ...
        private static async Task AutoMint()
        {
        ...
        public static void AutoMintDriver(string[] account, string fileName)
        {
        ...
        private static async Task AutoRef()
        {
        ...
        private static async Task CreateAccounts()
        {
            Console.WriteLine("Enter Work Time:");
            int workTimes = Convert.ToInt32(Console.ReadLine().Trim());
        ...
        private static async Task AutoTransfer()
        {
            Console.WriteLine("Enter Account Path:");
            ...
            {
                var username = account.Split('|').First();
                await AutoTransferDriver(username, wallet);
            }
        }

        public static async Task AutoTransferDriver(string username, string wallet)
        {
            var loginUrl = "https://www.reddit.com/login/?dest=https%3A%2F%2Fwww.reddit.com%2F";
            try
            {
                Console.WriteLine($"Working on Username: {username}\n");
                ...
                ((IJavaScriptExecutor)_undetectedDriver).ExecuteScript("document.querySelector('._6opQAE7SUXi-Fy7P3vItL').click()");
                //   IWebElement createAvtBtn = GetWebElementUntilSuccess(By.CssSelector("._6opQAE7SUXi-Fy7P3vItL"));
                ...
                SaveUrl(info, "Transfered", fileName);
                TotalRefSucceed += 1;
        ...
        public static async Task InputEmailOriginDriver(RedditInputModel inputModel)
        {
        ...
        public static async Task CreateAccountDriver(RedditInputModel inputModel, string fileName, int sleepTime = 30000)
        {
            var registerUrl = "https://www.reddit.com/register/";
            try
            {
        ...
        private static IWebElement GetWebElementUntilSuccess(By by, int maxWork = 10)
        {
            var workTime = 0;
